"""Temperature sensor platform for the Nobø Ecohub (Simple) integration.

Forked from the official integration; reads zone-component temperatures. Uses
the reconciler's dispatcher signal for updates (via ``NoboBaseEntity``).
"""
...
async def async_setup_entry(
    ...
    async_add_entities(
        NoboTemperatureSensor(reconciler, component["serial"])
...
class NoboTemperatureSensor(NoboBaseEntity, SensorEntity):
    ...
    def __init__(self, reconciler, serial: str) -> None:
        ...
        suggested_area = None
        if zone_id != "-1" and zone_id in hub.zones:
